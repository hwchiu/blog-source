...
Introduction
------------
- Event 提供一個介面 (EventCenter) 給上層的 AsyncMessenger 使用
- 真正實現 Async event I/O 的方法必須要繼承此介面，並且實作所有功能
- 目前提供的有
    - DPDK
    - EPOLL
    - KQUEUE
...
EventCallBack
-------------
- 基本上就是當事件發生時，要怎處理的介面雛形，所有的 CallBack 都必須要繼承此介面同時實作 `do_request`， 此 function 會得到一個 input，告知哪一個 fd 有事件發生了，然後實作對應的事情即可
...
EventDriver
-----------
- 這邊是真正底層 Event 實現的介面，DPDK/EPOLL/KQUEUE/SELECT都必須要實現此介面
```c++
0068 /*
...
```
##### init
- 進行一些初始化的動作，以 **select**為範例，可以將 read/write 的 FD 都初始化
##### add_event
- 加入一個新的 fd 到需要觀察的清單中，而 mask 則是有下列可能
- 以 **select** 為範例，就是將該 fd 加入到 fd set 中，同時更新當前最大 fd 數值
```c++
...
##### event_wait
- 這邊是等待 event 觸發的地方，使用者會傳入一個含有 **FiredFileEvent** 的 vector以及 timeout 的時間
- 若這次等待中，有任何 event 被觸發，都將其放入到 input 的 vector 中即可
...
##### need_wakup
- 預設是回傳 True，目前只有 DPDK 有重新定義，原因是因為 DPDK 跟其他三者 wait 的模式不相同，主要是一直依賴 polling 的方式去問。
...
- Event 分成
    ...
    - time event (多少 ms 後執行)
    - external event (馬上執行，可想成時間為 0 的 time event)
- 內部有再包含一個 Poller 的物件，主要是給 DPDK 使用。
- 針對非DPDK需要 wakeup 類型的 driver，如 EPOLL/KQUEUE/SELECT，實作了一個 read/write 的通知事件，為了避免 driver 卡在 wait 事件中，會透過 `pipe` 於本地創立一個專用的 FD，針對其 read fd 創造一個簡單的 read handler，單純讀取而已。
...
#### init
- 根據當前的 type 跟系統平台決定要使用何種 driver
    - dpdk
    - epool
    - kqueue
    - select
- 假設上層選擇的是 RDMA 的話，由於 control path 還是走系統，所以這邊可能會選擇 epool/kqueue/select
- 呼叫 driver 的 init
